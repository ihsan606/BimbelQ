...
export default function CreateMapel({ errors, session }) {

    const [ mapelName, setMapelName] = useState("");
    const [validation, setValidation] = useState([]);

    const submitMapel = async (e)=>{
        e.preventDefault();

        const formData = new FormData();

        //append data to formData
        formData.append('mapels_name', mapelName);

        await axios.post('/mapels', formData)
                            ...
                            {validation.mapels_name && (
                                <div className="bg-white text-center py-1 lg:px-4">
                                    <div className="p-2 bg-yellow-800 items-center text-yellow-100 leading-none lg:rounded-full flex lg:inline-flex" role="alert">
                                        ...
                                        <span className="font-semibold mr-2 text-left flex-auto">{validation.mapels_name[0]}</span>
